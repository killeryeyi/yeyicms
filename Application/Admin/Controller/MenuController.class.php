...
class MenuController extends GlobalController
{
    ...
    public function m_list()
    {
        ...
        $map = "";
        if (I("title") != "") {
            $map['title'] = array("like", "%" . I("title", "", trim) . "%");
            $this->assign("title", I("title", "", trim));
        }
        if (I("id") != "") {
            //$map['pid']=array("eq",I("id","",intval));
            $map[] = "pid =" . I("id") . " or id=" . I("id");
            $this->assign("id", I("id", "", intval));
        }
        $count = $Data->where($map)->count();// 查询满足要求的总记录数 $map表示查询条件
        $Page = new Page($count, 15);// 实例化分页类 传入总记录数
        $show = $Page->show();// 分页显示输出
        // 进行分页数据查询
        $list = $Data->where($map)->order('id asc,pid asc')->limit($Page->firstRow . ',' . $Page->listRows)->select(); // $Page->firstRow 起始条数 $Page->listRows 获取多少条
        //dump($this->tree_node($list)[0]);
        $pid = $Data->where(array('pid=0'))->select();
        $this->assign("caidan", $pid);
    ...
    public function m_add()
    {
        if ($_POST) {
            ...
            $node = M("admin_rule")->order('sort asc')->select();
    ...
    public function m_edit()
    {
        ...
        if ($_POST) {
            ...
            $url = M("admin_rule")->where($map)->find();
            if ($url) {
                $this->error("url已经存在");
            }
